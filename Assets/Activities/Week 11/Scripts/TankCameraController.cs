...
namespace GAD176.WeeklyActivities.WeekEleven
{
    ...
    // once we have our singleton class set up, lets turn the tank camera controller into a singleton instead of a monobehaviour
    public class TankCameraController : Singleton<TankCameraController>
    {
        ...
        public void OnGameStart()
        {
            ...
            if (!cam)
            {
                // here lets search the scene for a camera using find object of type.
                cam = FindObjectOfType<Camera>();

                // no camera in the scene. So Let's create one.
                if (cam == null)
                {
                    // here lets access the camera prefab from the game data instance.
                    // and lets call the Instantiate function using the camera prefab.
                    cameraParent = Instantiate(GameData.Instance.CameraPrefab);
                    // finally lets do a find object of type again and search for the camera script.
                    cam = FindObjectOfType<Camera>();
                }
            }
